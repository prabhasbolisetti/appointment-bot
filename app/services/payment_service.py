...
def get_payment_status(appointment_id: str) -> dict:
    ...
    db = get_db()
    # There may be multiple payment records for the same appointment (retries,
    # refunds, etc.). Fetch the most recent one by created_at.
    response = (
        db.table("payments")
        .select("*")
    ...
    # response.data is expected to be a list; take the first (most recent)
    payment = response.data[0] if isinstance(response.data, list) else response.data
    return {
        "appointment_id": appointment_id,
...
def refund_payment(appointment_id: str, amount: float = None) -> dict:
    ...
    try:
        # Get most recent payment for the appointment
        payment_response = (
            db.table("payments")
            .select("*")
            .eq("appointment_id", appointment_id)
        ...
        payment = payment_response.data[0] if isinstance(payment_response.data, list) else payment_response.data
